...
public class Shelf {
    ...
    public static boolean addBook(int id, String author, String title, int pages ){
        // TODO
        if (isIDUnique(id)&&title.length()>1&&pages>0){
            theBooks.add(new Book(id,author,title,pages));
            return true;
        }else {
    ...
    public static boolean addBook(Book book){
        ...
        // TODO
        if (!isIDUnique(book.getId())) return false;
        if (book.getTitle().isEmpty()) return false;
        if (book.getPages()<=0) return false;
        theBooks.add(book);
        return true;
    }
    ...
    public static String getTitleByID(int id) {
        ...
        for (Book i:theBooks){
            if (i.getId()==id)return i.getTitle();
            }
    ...
    public static Book findBookByPartialTitle(String title) {
       ...
       for (Book i:theBooks){
           if (i.getTitle().toLowerCase().contains(title.toLowerCase())) return i;
       }
    ...
    public static void removeBook(String author) {
        ...
        for (Book i:theBooks){
            if (i.getAuthor().toLowerCase().equals(author.toLowerCase())) theBooks.remove(i);
        }
