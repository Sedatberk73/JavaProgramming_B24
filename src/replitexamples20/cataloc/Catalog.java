...
public class Catalog {
    ...
    public static void main(String[] args) {
        catalogMain ctlmain=new catalogMain("ser");
        ctlmain.aysim="derger";
    }
...
String reverse="";
            for (int i=0;i<items.size();i++){
                reverse+=items.get(i)+"-"+prices.get(i)+"-"+monthlyPayments.get(i)+"\n";
            }
    ...
    public String getItemDetails(String item) {
        ...
        for (int i=0;i<items.size();i++){
            if(item.equals(items.get(i))){
                return rever+=items.get(i)+"-"+prices.get(i)+"-"+monthlyPayments.get(i);
            }
...
loadWholeCatalog();
        ...
        ArrayList<String> itemLessThanMonthlyPrice=new ArrayList<>();

        for(int i=0;i<monthlyPayments.size();i++){
            String eacitem="";
            if (price>=monthlyPayments.get(i)){
                eacitem+=items.get(i)+"-"+prices.get(i)+"-"+monthlyPayments.get(i);
                itemLessThanMonthlyPrice.add(eacitem);
            }
        ...
        return itemLessThanMonthlyPrice;
    }
